<div class="footer pt-10 bg-secondary">
    <div class="container">
        <div class="row mb-3">
            <div class="col-md-3">
                <h5 class="mb-3 text-white">@lang('em.useful_links')</h5>
                <div>
                    <ul class="list-unstyled">
                        <li><a class="text-white lh-lg"
                                href="{{ route('page', ['page' => 'about']) }}">@lang('em.about')</a>
                        </li>
                        <li><a class="text-white lh-lg" href="{{ route('events_index') }}">@lang('em.events')</a>
                        </li>
                        <li><a class="text-white lh-lg"
                                href="{{ route('page', ['page' => 'terms']) }}">@lang('em.terms')</a>
                        </li>
                        <li><a class="text-white lh-lg"
                                href="{{ route('page', ['page' => 'privacy']) }}">@lang('em.privacy')</a>
                    </ul>
                </div>
            </div>
            <div class="col-md-3">
                <h5 class="mb-3 text-white">@lang('em.social')</h5>
                <div>
                    <ul class="list-unstyled">
                        @if (setting('social.facebook'))
                            <li><a href="{{ 'https://www.facebook.com/' . setting('social.facebook') }}" target="_blank"
                                    class="text-white lh-lg">@lang('em.facebook')</a>
                        @endif
                        @if (setting('social.twitter'))
                            <li><a href="{{ 'https://twitter.com/' . setting('social.twitter') }}" target="_blank"
                                    class="text-white lh-lg">@lang('em.twitter')</a>
                        @endif
                        @if (setting('social.instagram'))
                            <li><a href="{{ setting('social.instagram') }}" target="_blank"
                                    class="text-white lh-lg">@lang('em.instagram')</a>
                        @endif
                        @if (setting('social.linkedin'))
                            <li><a href="{{ setting('social.linkedin') }}" target="_blank"
                                    class="text-white lh-lg">@lang('em.linkedin')</a>
                        @endif
                    </ul>
                </div>
            </div>
            <div class="col-md-3">
                <h5 class="mb-3 text-white">@lang('em.contact')</h5>
                <div>
                    <ul class="list-unstyled">
                        <li><a href="{{ route('contact') }}" class="text-white lh-lg">@lang('em.contact_send_message')</a>
                        </li>
                        <li><a href="{{ route('contact') }}" class="text-white lh-lg">@lang('em.contact_find_us')</a>
                        </li>
                    </ul>
                </div>
            </div>
                    ...
                    @foreach (lang_selector() as $val)
                        <li class="list-group-item bg-secondary border-0 p-1">
                            <a class="text-center col-grey px-2 text-white {{ $val == config('app.locale') ? 'active' : '' }}"
                                href="{{ route('change_lang', ['lang' => $val]) }}">
                                @lang('em.lang_' . $val)
                            </a>
                        </li>
                    @endforeach
        ...
        <div class="row">
            <div class="col-md-12 col-12 text-center">
                <p class="fs-6 text-gray-500 pb-3">
                    <span>©</span> {{ date('Y') }}
                    <a
                        href="{{ static_url() }}">{{ setting('site.site_name') ? setting('site.site_name') : config('app.name') }}</a><br>
