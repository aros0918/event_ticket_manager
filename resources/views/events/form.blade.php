...
@section('title')
    @if (empty($event))
        @lang('em.create_event')
    @else
        @lang('em.update_event')
    @endif
@endsection
                    ...
                    <div>
                        <h1 class="mb-0 fw-bold h2">
                            @if (empty($event))
                                @lang('em.create_event')
                            @else
                                @lang('em.update_event') - {{ $event->title }}
                            @endif
                        </h1>
                    </div>
                    <div class="d-flex">
                        <a class="btn btn-secondary btn-block" href="{{ route('myevents_index') }}"><span><i class="fas fa-xmark"></i> @lang('em.cancel')</span></a>  
                    </div>
                </div>
...
<script type="text/javascript" src="{{ static_asset('js/events_manage.js') }}"></script>
@stop
