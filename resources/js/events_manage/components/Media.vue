        ...
        <div class="panel-group">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <form
                        ref="form"
                        @submit.prevent="cropThumbnailPoster"
                        method="POST"
                    ...
                    >
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                <label class="form-label mb-0">{{
                                    trans("em.thumbnail_image")
                                }}</label>

                                <p class="mb-2 mt-0">
                                    <small class="text-muted"
                                        >{{ trans("em.zoom_info") }}
                                        {{ trans("em.drag_info") }}</small
                                    >
                                </p>
                                <croppa
                                    v-model="thumbnail_croppa"
                                    :placeholder="
                                        trans('em.drag_drop') +
                                        ' ' +
                                        trans('em.or') +
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                ...
                                <span
                                    v-show="errors.has('poster')"
                                    class="help text-danger"
                                    >{{ errors.first("poster") }}</span
                                >

                                <p class="mb-0 text-muted">
                                    {{ trans("em.poster_info") }}
                                </p>
                            </div>
                        </div>
                            ...
                            <div class="w-100">
                                ...
                                >
                                    ...
                                    >
                                        <button
                                            type="button"
                                            class="btn-sm btn-remove-image bg-light-danger text-danger"
                                            @click="deleteGalleryImages(image)"
                                        >
...
export default {
    ...
    methods: {
        ...
        async cropThumbnailPoster() {
            ...
            this.thumbnail = await this.thumbnail_croppa.generateDataUrl(
                "image/jpeg"
            );
            this.poster = await this.poster_croppa.generateDataUrl(
                "image/jpeg"
            );
        ...
        formSubmit(event) {
            ...
            let post_url = route("myevents_store_media");
            ...
            axios
                ...
                .then((res) => {
                    ...
                    if (res.data.status) {
                        ...
                        this.multiple_images = this.images.images
                            ? JSON.parse(this.images.images)
                            : [];
        ...
        editMedia() {
            if (Object.keys(this.event).length > 0) {
                this.thumbnail_preview = this.event.thumbnail
                    ? "/storage/" + this.event.thumbnail
                    : null;
                this.poster_preview = this.event.poster
                    ? "/storage/" + this.event.poster
                    : null;
                this.multiple_images = this.event.images
                    ? JSON.parse(this.event.images)
                    : [];
            }
        },
        ...
        deleteGalleryImages(image = null) {
            this.showConfirm(trans("em.delete")).then((res) => {
                if (res) {
                    // prepare form data for post request
                    let post_url = route("delete_image");
                    let post_data = {
