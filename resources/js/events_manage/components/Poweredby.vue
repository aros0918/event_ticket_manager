            ...
            <div class="card-body p-4">
                <div v-if="event.publish == 1">
                    <span class="text-danger h4">{{
                        trans("em.unpublish_event")
                    }}</span>
                    <p class="text-danger">
                        {{ trans("em.unpublish_event_ie") }}
                    </p>
                </div>
                ...
                <button
                    ...
                    :disabled="
                        Object.keys(this.is_publishable).length != 4 &&
                        event.publish == 0
                            ? true
                            : false
                    "
                    :class="{
                        'btn-outline-danger': event.publish == 1,
                        'btn-outline--success': event.publish != 1,
                    }"
                    @click="publishEvent()"
                >
                    ...
                    if (res.data.status) {
                        if (this.event.publish == 1)
                            this.showNotification(
                                "success",
                                trans("em.event_unpublished")
                            );
                        else
                            this.showNotification(
                                "success",
                                trans("em.event_published")
                            );
    ...
    mounted() {
        ...
        if (event_step) {
            ...
            this.getMyEvent().then(function (response) {
                $this.is_publishable = $this.event.is_publishable
                    ? JSON.parse($this.event.is_publishable)
                    : [];
            });
        }
    },
