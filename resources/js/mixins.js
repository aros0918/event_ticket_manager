    ...
    methods: {
        ...
        convert_date_to_local_format(date) {
            ...
            if (timezone_conversion <= 0)
                ...
                return moment(date)
                    .tz(local_tz)
                    .format(date_format.vue_date_format);
        },
        ...
        convert_time_to_local(date, time, format) {
            ...
            if (timezone_conversion <= 0) {
                ...
                if (typeof format !== "undefined")
                    return moment(date + " " + time)
                        .tz(local_tz)
                        .format(date_format.vue_time_format);
        ...
        counthours(start_time, end_time, repetitive) {
            var minutes_diff = moment(end_time, "YYYY-MM-DD HH:mm:ss").diff(
        ...
        changeTimeFormat(time) {
            return moment(time, date_format.vue_time_format).format(
                date_format.vue_time_format
            );
        },
        ...
        dateToFullDate(date, monthYear) {
            return moment(
                moment(monthYear).format("YYYY-MM") + "-" + date
            ).format("dddd LL");
        },
        ...
        getMyEvent() {
            let promise = null;
            let post_url = route("get_myevent");
            let $this = this;
        ...
        positiveInteger(value = null) {
            if (value > 0 && Number.isInteger(value) == true) return value;
        ...
        userTimezone(date = null, format = null) {
            if (timezone_conversion <= 0) return moment(date, format);
            else return moment.tz(date, format, timezone_default).tz(local_tz);
        },
        ...
        serverTimezone(date = null, format = null) {
            if (timezone_conversion <= 0) return moment(date, format);
            else return moment.tz(date, format, local_tz).tz(timezone_default);
        },
