...
class SettingsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Site Name", "value" => "Event Laravel", "details" => null, "type" => "text", "order" => "1", "group" => "Site",])->save();
        }

        $setting = $this->findSetting("site.site_slogan");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Site Slogan", "value" => "event management", "details" => null, "type" => "text", "order" => "2", "group" => "Site",])->save();
        }

        $setting = $this->findSetting("site.site_footer");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Extra Footer Credits", "value" => "Event Laravel <strong>v1.0<strong>. Product by <a href='https://github.com/dybbuk-dev' target='_blank'>Louis Brandt</a>", "details" => null, "type" => "text", "order" => "2", "group" => "Site",])->save();
        }

        $setting = $this->findSetting("site.logo");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Site Logo", "value" => "settings\April2024\caHN3a5rqGMvCGYwpusd.jpg", "details" => null, "type" => "image", "order" => "3", "group" => "Site",])->save();
        }

        $setting = $this->findSetting("site.site_favicon");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Site Favicon", "value" => "settings\April2024\caHN3a5rqGMvCGYwpusd.jpg", "details" => null, "type" => "image", "order" => "4", "group" => "Site",])->save();
        }

        $setting = $this->findSetting("seo.meta_title");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Meta Title", "value" => "Event Laravel", "details" => null, "type" => "text", "order" => "5", "group" => "SEO",])->save();
        }

        $setting = $this->findSetting("seo.meta_description");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Meta Description", "value" => "event laravel", "details" => null, "type" => "text", "order" => "7", "group" => "SEO",])->save();
        }

        $setting = $this->findSetting("social.facebook");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Facebook Page Username", "value" => "event laravel", "details" => null, "type" => "text", "order" => "8", "group" => "Social",])->save();
        }

        $setting = $this->findSetting("social.twitter");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Twitter Account Username", "value" => "event laravel", "details" => null, "type" => "text", "order" => "9", "group" => "Social",])->save();
        }

        $setting = $this->findSetting("social.instagram");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Instagram URL", "value" => "https://www.instagram.com", "details" => null, "type" => "text", "order" => "10", "group" => "Social",])->save();
        }

        $setting = $this->findSetting("social.linkedin");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Linkedin URL", "value" => "https://www.linkedin.com", "details" => null, "type" => "text", "order" => "11", "group" => "Social",])->save();
        }
        ...
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Email", "value" => "info@com", "details" => null, "type" => "text", "order" => "14", "group" => "Contact",])->save();
        }
        ...
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Admin Background Image", "value" => "", "details" => null, "type" => "image", "order" => "23", "group" => "Admin",])->save();
        }

        $setting = $this->findSetting("admin.title");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Admin Title", "value" => "Event Laravel", "details" => null, "type" => "text", "order" => "24", "group" => "Admin",])->save();
        }

        $setting = $this->findSetting("admin.description");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Admin Description", "value" => "Event Laravel Admin Panel", "details" => null, "type" => "text", "order" => "25", "group" => "Admin",])->save();
        }
        ...
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Admin Icon Image", "value" => "settings\April2024\caHN3a5rqGMvCGYwpusd.jpg", "details" => null, "type" => "image", "order" => "27", "group" => "Admin",])->save();
        }
        ...
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Mail Sender Email", "value" => "event@gmail.com", "details" => null, "type" => "text", "order" => "42", "group" => "Mail",])->save();
        }

        $setting = $this->findSetting("mail.mail_sender_name");
        if (!$setting->exists) {
            $setting->fill(["display_name" => "Mail Sender Name", "value" => "Event Laravel", "details" => null, "type" => "text", "order" => "43", "group" => "Mail",])->save();
        }
        ...
        if (!$setting->exists) {
            $setting->fill([
                "display_name" => "Timezone Conversion",
                "value" => "1",
                "details" => json_encode([
                    "validation" => [
                        "rule" => "in:0,1,on,off"
