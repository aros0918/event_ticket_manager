...
class BookingsController extends Controller
{
    ...
    public function book_tickets(Request $request)
    {
        ...
        return response([
            'status' => true,
            'url' => Auth::user()->hasRole('admin') ? static_url(config('custom.route.admin_prefix') . '/bookings') : route('mybookings_index'),
            'message' => __('em.congrats') . ' ' . __('em.booking') . ' ' . __('em.successful'),
        ], Response::HTTP_OK);
    }
        ...
        if (Auth::check()) {
            ...
            if (Auth::user()->hasRole('admin')) {
                ...
                $request->validate([
                    ...
                    'customer_id.*' => __('em.customer') . ' ' . __('em.required'),
                ]);
